...
class API {
  ...
  public static async getRequest(endpoint: string, data = {}): Promise<any> {
    let res = await request(this.route, endpoint, "get", data);
    return res;
  }

  public static async postRequest(endpoint: string, data = {}): Promise<any> {
    let res = await request(this.route, endpoint, "post", data);
    return res;
  }

  public static async patchRequest(endpoint: string, data = {}): Promise<any> {
    let res = await request(this.route, endpoint, "patch", data);
    return res;
  }

  public static async deleteRequest(endpoint: string, data = {}): Promise<any> {
